...
After $M$ simulations from root state $s_0$, the search policy $\pi$ is extracted directly from the **visit counts**, not the $Q$-values:
...
> **Key Theoretical Result:** The visit count distribution $\pi_{\text{MCTS}}$ is a **strictly improved policy** compared to the raw neural network prior $\mathbf{p}_\theta(s_0)$:
...
$$\mathcal{L}(\theta) = \underbrace{(z - v_\theta(s))^2}_{\text{Value MSE Loss}} - \underbrace{\pi_{\text{MCTS}}^T \log \mathbf{p}_\theta(s)}_{\text{Policy Cross-Entropy Loss}} + c \|\theta\|^2$$
...
$$\mathcal{D} = \left\{ \left( s_i, \pi_i, z_i \right) \right\}_{i=1}^B$$
where for transition $i$:
- $s_i$ is the board state.
- $\pi_i \triangleq \pi_{\text{MCTS}}(\cdot \mid s_i) \in \Delta^{K-1}$ is the search policy vector formed from MCTS visit counts:
...
Our mathematical goal is to:
1. Derive the joint loss function $\mathcal{L}(\theta) = (z - v_\theta(s))^2 - \pi_i^\top \log \mathbf{p}_\theta(s) + c \|\theta\|_2^2$ from Bayesian Maximum A Posteriori (MAP) principles.
2. Derive exact closed-form analytical gradients with respect to the network logits:
   $$\nabla_{\mathbf{z}_{\text{pol}}} \mathcal{L}_{\text{pol}} = \mathbf{p}_\theta(s) - \pi_i \quad \text{and} \quad \frac{\partial \mathcal{L}_{\text{val}}}{\partial z_{\text{val}}} = -2 (z - v_\theta)(1 - v_\theta^2)$$
3. Prove the MCTS Policy Improvement Theorem: $\mathbb{E}_{a \sim \pi}[Q(s, a)] \ge \mathbb{E}_{a \sim \mathbf{p}}[Q(s, a)]$, showing that MCTS acts as a strict Generalized Policy Iteration operator.

**Part 2: Explicit Assumptions & Regularity Conditions**
1. **Conditional Independence:** Conditioned on the state $s_i$ and network weights $\theta$, the observed game outcome $z_i$ and the search policy target $\pi_i$ are conditionally independent:
   $$p(z_i, \pi_i \mid s_i, \theta) = p(z_i \mid s_i, \theta) \cdot p(\pi_i \mid s_i, \theta)$$
...
1. **Policy Improvement (MCTS):** Given a current neural network prior $\mathbf{p}_\theta$, MCTS conducts $M$ simulations of forward lookahead tree search. Because MCTS aggregates deep tactical rollouts and propagates minimax backed-up values, the resulting visit count distribution $\pi_{\text{MCTS}}$ is a **strictly superior, tactically sounder policy** than the raw prior $\mathbf{p}_\theta$.
2. **Policy Evaluation & Distillation (Gradient Descent):** However, running 800 MCTS simulations at test time for every single move is computationally expensive. The dual-head loss acts as a **policy distillation and value regression engine**:
   - The cross-entropy loss $-\pi^\top \log \mathbf{p}$ trains the fast neural intuition $\mathbf{p}_\theta$ to mimic the deep tactical search $\pi_{\text{MCTS}}$. In information geometry, this minimizes the Kullback-Leibler divergence $D_{\text{KL}}(\pi \parallel \mathbf{p})$, projecting the tree search distribution onto the neural network manifold.
...
Given dataset $\mathcal{D} = \{(s_i, \pi_i, z_i)\}_{i=1}^B$, the posterior distribution of parameter vector $\theta$ is given by Bayes' Theorem:
$$p(\theta \mid \mathcal{D}) = \frac{p(\mathcal{D} \mid \theta) p(\theta)}{p(\mathcal{D})} \propto p(\theta) \prod_{i=1}^B p(z_i, \pi_i \mid s_i, \theta)$$
Applying conditional independence of the value outcome and search distribution:
$$p(\theta \mid \mathcal{D}) \propto p(\theta) \prod_{i=1}^B \left[ p(z_i \mid s_i, \theta) \cdot p(\pi_i \mid s_i, \theta) \right]$$
Taking the natural logarithm of both sides:
$$\ln p(\theta \mid \mathcal{D}) = \ln p(\theta) + \sum_{i=1}^B \ln p(z_i \mid s_i, \theta) + \sum_{i=1}^B \ln p(\pi_i \mid s_i, \theta) + \text{const} \quad \text{(Eq. 3.1)}$$
...
2. **Policy Head Log-Likelihood:**
   Under the Multinomial model with $M$ visits and normalized targets $\pi_i = \frac{\mathbf{N}_i}{M}$:
   ...
   $$\frac{1}{M} \ln p(\mathbf{N}_i \mid s_i, \theta) = \sum_{a=1}^K \pi_i(a) \ln p_\theta(a \mid s_i) + \text{const} = \pi_i^\top \log \mathbf{p}_\theta(s_i) + \text{const}$$
...
$$\ln p(\theta \mid \mathcal{D}) = \sum_{i=1}^B \left[ -(z_i - v_\theta(s_i))^2 + M \pi_i^\top \log \mathbf{p}_\theta(s_i) \right] - \frac{\|\theta\|_2^2}{2 \sigma_\theta^2} + \text{const}$$
...
$$\mathcal{L}(\theta) = \left( z - v_\theta(s) \right)^2 - \pi_{\text{MCTS}}^\top \log \mathbf{p}_\theta(s) + c \|\theta\|_2^2 \quad \text{(Eq. 3.2)}$$
...
Since $\pi$ is a valid probability distribution, $\sum_{k=1}^K \pi_k = \pi_i + \sum_{k \ne i} \pi_k = 1$:
$$\frac{\partial \mathcal{L}_{\text{pol}}}{\partial z_i} = -\pi_i + p_i (1) = p_i - \pi_i$$
In full vector form:
$$\nabla_{\mathbf{z}_{\text{pol}}} \mathcal{L}_{\text{pol}} = \mathbf{p}_\theta(s) - \pi_{\text{MCTS}} \quad \text{(Eq. 3.3)}$$
...
We now prove that the MCTS search policy $\pi_{\text{MCTS}}$ achieves an expected action value strictly greater than or equal to that of the neural network prior $\mathbf{p}$:
...
Define the cumulative distribution functions $F_{\pi}(m) \triangleq \sum_{k=1}^m \pi_{(k)}$ and $F_{\mathbf{p}}(m) \triangleq \sum_{k=1}^m p_{(k)}$.
...
$$F_{\pi}(m) \ge F_{\mathbf{p}}(m) \quad \forall m \in \{1, \dots, K\}$$
...
$$= \sum_{m=1}^{K-1} \left( F_{\pi}(m) - F_{\mathbf{p}}(m) \right) \left( Q_{(m)} - Q_{(m+1)} \right) + \left( F_{\pi}(K) - F_{\mathbf{p}}(K) \right) Q_{(K)}$$
Since $F_{\pi}(K) = F_{\mathbf{p}}(K) = 1$, the terminal term vanishes:
$$\sum_{k=1}^K (\pi_{(k)} - p_{(k)}) Q_{(k)} = \sum_{m=1}^{K-1} \underbrace{\left( F_{\pi}(m) - F_{\mathbf{p}}(m) \right)}_{\ge 0} \underbrace{\left( Q_{(m)} - Q_{(m+1)} \right)}_{\ge 0} \ge 0$$
Equality holds if and only if all action values are identical ($Q_{(1)} = Q_{(K)}$) or $\pi \equiv \mathbf{p}$.
Therefore, MCTS is a strictly monotonic policy improvement operator:
$$\pi_{\text{MCTS}} \succ \mathbf{p}_\theta(s) \quad \blacksquare$$
...
The policy target vector $\pi_\tau$ is defined by:
...
4. **Regime 4: Greedy Limit ($\tau \to 0$):**
   $$\lim_{\tau \to 0} \pi_\tau(a) = \begin{cases} 1.0 & \text{if } a = \arg\max_b N(s, b) = a_1 \\ 0.0 & \text{otherwise} \end{cases} \implies \pi_0 = [\mathbf{1.0}, \, \mathbf{0.0}, \, \mathbf{0.0}, \, \mathbf{0.0}]$$
...
From self-play tree search, the target search policy is $\pi_{\text{target}} = [0.70, 0.20, 0.10]$ and the terminal outcome is $z_{\text{target}} = +1.00$ (win).
Calculate:
...
3. **Analytical Gradients w.r.t. Logits:**
   - **Policy Logit Gradient (Eq. 3.3):**
     $$\nabla_{\mathbf{z}_{\text{pol}}} \mathcal{L}_{\text{pol}} = \mathbf{p} - \pi_{\text{target}}$$
...
**Problem:**
During self-play, AlphaZero adds Dirichlet noise $\eta \sim \operatorname{Dir}(\alpha)$ to the root node prior:
...
A sampled Dirichlet vector is $\eta = [0.10, \, 0.60, \, 0.30]$.

...
$$\pi_\theta(a \mid s) \triangleq \mathbb{P}(A_t = a \mid S_t = s; \theta)$$
where $\theta \in \mathbb{R}^d$ is a continuous parameter vector (e.g., neural network weights).

The central mathematical challenge is: **How can we compute the gradient of expected return with respect to policy parameters $\theta$ when the environment's transition dynamics $\mathcal{P}_{ss'}^a$ are completely unknown?**
...
The probability of trajectory $\tau$ occurring under policy parameters $\theta$ is:
...
$$J(\theta) \triangleq \mathbb{E}_{\tau \sim \pi_{\theta}} [R(\tau)] = \int P(\tau; \theta) R(\tau) d\tau$$

---

### 2.2 The Log-Derivative Trick (Likelihood Ratio / Score Function)

To optimize $J(\theta)$ via gradient ascent ($\theta \leftarrow \theta + \alpha \nabla_{\theta} J$), we differentiate the expectation:
...
- $\nabla_{\theta} \log \mu(s_0) = \mathbf{0}$ (initial state distribution does not depend on $\theta$).
- $\nabla_{\theta} \log \mathcal{P}(s_{t+1} \mid s_t, a_t) = \mathbf{0}$ (the laws of physics / environment dynamics do NOT depend on $\theta$!).
...
$$\mathbb{E} \left[ \nabla_{\theta} \log \pi_{\theta}(a_t \mid s_t) r_{k+1} \right] = \mathbf{0} \quad \forall k < t$$
...
$$\mathbb{E}_{a \sim \pi_{\theta}} \left[ \nabla_{\theta} \log \pi_{\theta}(a \mid s) b(s) \right] = \mathbf{0}$$

#### Proof
Expand the expectation over the discrete action space $\mathcal{A}$:
$$\mathbb{E}_{a \sim \pi_{\theta}} \left[ \nabla_{\theta} \log \pi_{\theta}(a \mid s) b(s) \right] = \sum_{a \in \mathcal{A}} \pi_{\theta}(a \mid s) \frac{\nabla_{\theta} \pi_{\theta}(a \mid s)}{\pi_{\theta}(a \mid s)} b(s)$$
$$= b(s) \sum_{a \in \mathcal{A}} \nabla_{\theta} \pi_{\theta}(a \mid s) = b(s) \nabla_{\theta} \left( \sum_{a \in \mathcal{A}} \pi_{\theta}(a \mid s) \right)$$
Because $\pi_{\theta}$ is a valid probability distribution, the sum of probabilities over all actions is always identically 1: $\sum_a \pi_{\theta}(a \mid s) = 1$.
The gradient of a constant is zero:
$$= b(s) \nabla_{\theta} (1) = b(s) \cdot \mathbf{0} = \mathbf{0} \quad \blacksquare$$
...
In vector notation:
$$\nabla_{\mu} \log \pi_\theta(\mathbf{a} \mid s) = \mathbf{\Sigma}^{-1} (\mathbf{a} - \mu(s)) \quad \blacksquare$$
...
In vector notation:
$$\nabla_{\mathbf{s}} \log \pi_\theta(\mathbf{a} \mid s) = \left( \frac{\mathbf{a} - \mu(s)}{\sigma(s)} \right)^{\odot 2} - \mathbf{1} \quad \blacksquare$$
...
- The score function $\nabla_{\theta} \log \pi_{\theta}(a \mid s)$ points in the direction that **increases the probability of action $a$** while simultaneously decreasing the probabilities of all competing actions (since probabilities must sum to 1).
...
- **Policy-Based (REINFORCE) Approach:** 
  - The actor delivers a full monologue with their natural expressive style $\pi_{\theta}$.
...
The policy is parameterized by a 2D logit vector $\theta = [\theta_0, \theta_1]^\top$:
$$\pi_{\theta}(a_i) \triangleq \frac{e^{\theta_i}}{e^{\theta_0} + e^{\theta_1}}$$

**Initial Parameters:**
$$\theta = \begin{bmatrix} 1.0000 \\ 2.0000 \end{bmatrix}$$
...
| :--- | :--- | :--- |
| $\theta$ | Policy Parameter Logits | $[\theta_0=1.0000, \theta_1=2.0000]^\top$ |
...
| $\nabla_{\theta} \log \pi(a_0)$ | Score Function Vector for $a_0$ | $[1 - \pi_0, -\pi_1]^\top$ |
...
| $\theta_{\text{new}}$ | Updated Parameters | $\theta + \alpha \mathbf{g}$ |
...
$$\nabla_{\theta} \log \pi(a_0) = \begin{bmatrix} +0.73106 \\ -0.73106 \end{bmatrix}$$
...
$$\mathbf{g} = A_0 \nabla_{\theta} \log \pi(a_0) = 5.0000 \times \begin{bmatrix} +0.73106 \\ -0.73106 \end{bmatrix} = \begin{bmatrix} +3.65530 \\ -3.65530 \end{bmatrix}$$
...
$$\theta_{\text{new}} = \theta + \alpha \mathbf{g} = \begin{bmatrix} 1.0000 \\ 2.0000 \end{bmatrix} + 0.1000 \begin{bmatrix} +3.65530 \\ -3.65530 \end{bmatrix}$$
$$\theta_{\text{new}} = \begin{bmatrix} 1.0000 + 0.36553 \\ 2.0000 - 0.36553 \end{bmatrix} = \mathbf{\begin{bmatrix} 1.36553 \\ 1.63447 \end{bmatrix}}$$

---

### 5.7 Step 5: Verify Updated Policy Probabilities

Compute new exponentials with updated $\theta_{\text{new}}$:
...
| Quantity | Initial State ($\theta$) | Gradient Component | Updated State ($\theta_{\text{new}}$) | Delta Change |
| :---: | :---: | :---: | :---: | :---: |
...
**Problem:**
Consider a single-state decision problem with two actions $\mathcal{A} = \{a_0, a_1\}$. The current policy parameter vector is $\theta = [\theta_0, \theta_1]^\top$, giving action probabilities $\pi(a_0) = 0.40$ and $\pi(a_1) = 0.60$. The deterministic returns are $R(a_0) = 10.0$ and $R(a_1) = 2.0$.
...
4. The mean squared error loss gradient for the critic parameters if $V_\phi(s) = \phi^\top \mathbf{x}(s)$ with $\mathbf{x}(s_0) = [1, 0]^\top, \mathbf{x}(s_1) = [0, 1]^\top, \mathbf{x}(s_2) = [1, 1]^\top$.
...
The critic objective per transition is $\mathcal{L}(\phi) = \frac{1}{2} (G_t - V_\phi(s_t))^2$.
Gradient with respect to $\phi$:
$$\nabla_{\phi} \mathcal{L}_t = -(G_t - V_\phi(s_t)) \nabla_{\phi} V_\phi(s_t) = -\hat{A}_t \mathbf{x}(s_t)$$
...
$$\nabla_{\phi} \mathcal{L} = \sum_{t=0}^2 \nabla_{\phi} \mathcal{L}_t = \begin{bmatrix} -2.9000 + 0 - 3.0000 \\ 0 - 2.0000 - 3.0000 \end{bmatrix} = \mathbf{\begin{bmatrix} -5.9000 \\ -5.0000 \end{bmatrix}} \quad \blacksquare$$

---

### Illustration 4: Analytical Variance-Minimizing Baseline vs. State-Value Baseline
**Problem:**
In a contextual bandit with three discrete actions $\mathcal{A} = \{a_0, a_1, a_2\}$, the current policy probabilities are $\pi = [0.20, 0.50, 0.30]^\top$, and the deterministic rewards are $\mathbf{R} = [10.0, 4.0, 2.0]^\top$.
1. Compute the classical state-value baseline $V = \sum_a \pi(a) R(a)$.
2. Compute the exact variance-minimizing baseline $b^* = \frac{\mathbb{E}[\|\nabla_{\theta} \log \pi(A)\|^2 R(A)]}{\mathbb{E}[\|\nabla_{\theta} \log \pi(A)\|^2]}$ derived in Section 2.7.
...
For a 3-class softmax policy parametrized by $\theta \in \mathbb{R}^3$, the score vector for action $a$ is $\nabla_{\theta} \log \pi(a) = \mathbf{e}_a - \pi$.
The squared Euclidean norm is:
$$\|\nabla_{\theta} \log \pi(a)\|^2 = (1 - \pi_a)^2 + \sum_{k \ne a} \pi_k^2$$
- For $a_0$ ($\pi_0 = 0.20$):
...
Numerator:
$$\mathbb{E}[\|\nabla_{\theta} \log \pi(A)\|^2 R(A)] = \sum_{a} \pi(a) \|\mathbf{g}_a\|^2 R(a)$$
...
Denominator:
$$\mathbb{E}[\|\nabla_{\theta} \log \pi(A)\|^2] = \sum_{a} \pi(a) \|\mathbf{g}_a\|^2$$
...
$$b^* = 5.1419 > V = 4.6000$$
The optimal baseline $b^*$ places heavier weight on actions that produce larger score vector norms $\|\nabla_{\theta} \log \pi(a)\|^2$. Action $a_0$ has probability $0.20$ (low), giving it a large score norm ($0.98$), and it yields the highest reward ($10.0$). Consequently, the variance-minimizing baseline shifts upward toward $10.0$ ($5.1419 > 4.6000$) to suppress the high-norm gradient variance of action $a_0$. $\blacksquare$
...
1. Exact numerical reproduction of Part 5 Softmax Policy hand calculations:
   ...
   - Updated parameters $\theta_{\text{new}} = [1.36553, 1.63447]$
